...
@ApiResponses(value = {
		@ApiResponse(responseCode = "400", description = "Erro de validação ou requisição inválida", 
				content = @Content(mediaType = "text/plain", schema = @Schema(implementation = String.class), examples = @ExampleObject(value = "Dados inválidos!!!"))) })
public class AuthController {
	...
	public ResponseEntity<String> PostSignin(@RequestBody SigninDTO signin) {
		...
		return ResponseEntity.ok(JwtUtil.generateToken(signin.getEmail()));
	}
